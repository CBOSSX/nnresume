...
async function startCommand(appRoot, argv, options = {}) {
  ...
  if (flags.background) {
    ...
    const child = spawn(process.execPath, [path.join(appRoot, "bin", "nnresume.js"), "start", ...forwarded], {
      cwd: workspaceRoot,
      detached: true,
      stdio: "ignore",
    });
    child.unref();
    const backgroundPort = Number(flags.port || process.env.PORT || 4173);
    process.stdout.write(`╭─ nnresume 后台启动中 ─────────────────────╮\n`);
    process.stdout.write(`│ 地址      http://127.0.0.1:${backgroundPort}\n`);
    process.stdout.write(`│ 工作区    ${workspaceRoot}\n`);
    process.stdout.write(`╰─ 可关闭当前终端；如端口被占用请换 --port ─╯\n`);
    return child;
  }
