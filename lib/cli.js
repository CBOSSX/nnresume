...
function buildBackgroundArguments(appRoot, workspaceRoot, flags, port) {
  const args = [path.join(appRoot, "bin", "nnresume.js"), "start", workspaceRoot, `--port=${port}`];
...
async function startCommand(appRoot, argv, options = {}) {
  ...
  if (flags.background) {
    const childArgs = buildBackgroundArguments(appRoot, workspaceRoot, flags, port);
    ...
    await new Promise((resolve, reject) => {
      ...
      const fail = (error, terminate = true) => {
        ...
        if (terminate && child.pid && child.exitCode === null) child.kill("SIGTERM");
        reject(error);
      };
      ...
      timeout = setTimeout(() => fail(new Error("后台启动超时，请检查端口是否可用")), 10000);
      child.once("error", fail);
      child.once("exit", (code) => fail(new Error(`后台启动失败（退出码 ${code ?? "unknown"}）`), false));
      child.on("message", (message) => {
        if (message?.type === "nnresume:error") fail(new Error(message.message || "后台启动失败"));
        if (message?.type === "nnresume:ready") succeed(message);
      });
    });
    if (child.connected) child.disconnect();
    child.unref();
    process.stdout.write(`╭─ nnresume 后台编辑器已启动 ───────────────╮\n`);
    process.stdout.write(`│ 地址      http://127.0.0.1:${port}\n`);
    process.stdout.write(`│ 工作区    ${workspaceRoot}\n`);
    process.stdout.write(`╰─ 可关闭当前终端 ─────────────────────────╯\n`);
    return child;
  }
  ...
  if (typeof process.send === "function") process.send({ type: "nnresume:ready", url, workspaceRoot });
  process.stdout.write(`╭─ nnresume 本地编辑器 ─────────────────────╮\n`);
  process.stdout.write(`│ 地址      ${url}\n`);
  process.stdout.write(`│ 工作区    ${workspaceRoot}\n`);
  process.stdout.write(`│ 后台运行  npx nnresume --background\n`);
  process.stdout.write(`╰─ 按 Ctrl+C 停止当前服务 ─────────────────╯\n`);
